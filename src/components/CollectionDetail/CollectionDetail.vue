      ...
      <el-row>
        ...
        <el-form style="padding-left:40px;" :model="form" label-width="150px">
          <el-form-item prop="user" label="指派采集人：" v-if = "$route.query.type == 'edit'">
            <el-checkbox-group type="small" v-model="form.collector">
...
export default {
  ...
  data () {
    return {
      navList:[
        ...
        {path:'/task/collectionmanagement',name:'开发楼盘采集'},
  ...
  methods: {
      ...
      this.$http('/taskManage/editCollection',{body},{},{},'post').then((res) => {
        if(res.data.code == '0'){
          if(res.data.response.status == 1){
            ...
            _this.$router.push('/task/collectionmanagement')
          }else{
    ...
    //返回
    back(){
      this.$router.push('/task/collectionmanagement')
    }
  },
}
</script>
